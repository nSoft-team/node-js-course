...
// Creating the router. Note: const server = express(); will also work but express.Router() is more suitable for controller's inner routes:
...
router.get("/:id([0-9]+)", async (request, response) => {
  // ([0-9]+) checks for a numeric id. If not numeric, this route is skipped.
  try {
    // Data:
...
router.patch("/:id([0-9]+)", async (request, response) => {
  try {
    ...
    if (!updatedProduct) return response.sendStatus(404);
